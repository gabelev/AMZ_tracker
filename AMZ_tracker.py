        ...
        for old_cell in row:
            if pd.notnull(old_cell):
                ...
                if number in tracking_asin:
                    tracking = 'yes'
                x = index, rank, number, rank_type, str(today), tracking
                results.append(x)
    name = output_name + "_" + str(today) + '.csv'
    myfile = open(name, 'wb')
...
if __name__ == "__main__":
    ...
    business_result = run_spider_run(Search_Business, business_tracking)
